    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                ...
                <h6>Form Edit Template Soal :</h6>

                <form action="{{ route('template-soal.update', $template_soal->id) }}" enctype="multipart/form-data" method="POST">
                    @csrf
                    @method('put')
                    @if ($idGuru)
                        ...
                        <div class="col-4">
                            ...
                            <select name="kegiatan_id" class="form-select @error('kegiatan_id') is-invalid @enderror" id="">
                                @foreach ($kegiatan as $item)
                                    <option value="{{ $item->id }}" {{ $item->id == $template_soal->kegiatan_id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                @endforeach
                            </select>
                            @error('kegiatan_id')
